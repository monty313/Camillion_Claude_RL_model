# ADX-DI alignment alphas (slots 16/17) + the v1.6.0 OHLC observation block + the DI side-channel.
# Locks: the -DI vs +DI agreement logic, slot wiring, that DI feeds the alphas via aux (not the obs),
# that the raw OHLC block is present + leak-free, and that the obs shape is 499.
import numpy as np
import pandas as pd
