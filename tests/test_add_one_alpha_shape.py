# Audit S7.31: registering one alpha keeps shape (553,), fills its slot + mask,
...
def test_add_one_alpha_keeps_shape():
    ...
    o0, _ = e0.reset()
    assert o0.shape == (553,) and o0[OC.BLOCK_SLICES["alpha_mask"]].sum() == 0
    reg = AlphaRegistry(); slot = reg.register(_Buy("dummy"))
    e1 = TradingEnv(ind, cl, t, reg, warmup=210)
    o1, _ = e1.reset()
    assert o1.shape == (553,)                              # shape unchanged
