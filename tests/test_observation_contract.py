# Test 8: the observation shape contract holds (461 float32 finite) and stays
...
def test_total_size_461():
    assert C.OBS_TOTAL_SIZE == 461 and C.OBS_SHAPE == (461,)
    assert len(OC.FEATURE_NAMES) == 461


def test_block_sizes_sum_to_total():
    assert sum(s.stop - s.start for s in OC.BLOCK_SLICES.values()) == 461


def test_zeros_observation_valid():
    z = B.zeros()
    assert z.shape == (461,) and z.dtype == np.float32 and np.all(np.isfinite(z))
