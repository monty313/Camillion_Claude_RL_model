# Test 8: the observation shape contract holds (526 float32 finite) and stays
...
def test_total_size_479():
    assert C.OBS_TOTAL_SIZE == 526 and C.OBS_SHAPE == (526,)
    assert len(OC.FEATURE_NAMES) == 526


def test_block_sizes_sum_to_total():
    assert sum(s.stop - s.start for s in OC.BLOCK_SLICES.values()) == 526


def test_zeros_observation_valid():
    z = B.zeros()
    assert z.shape == (526,) and z.dtype == np.float32 and np.all(np.isfinite(z))
...
def test_full_build_finite_and_shaped():
    ...
    assert obs.shape == (526,) and obs.dtype == np.float32 and np.all(np.isfinite(obs))
