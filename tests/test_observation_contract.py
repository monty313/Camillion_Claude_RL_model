# Test 8: the observation shape contract holds (553 float32 finite) and stays
...
def test_total_size_479():
    assert C.OBS_TOTAL_SIZE == 553 and C.OBS_SHAPE == (553,)
    assert len(OC.FEATURE_NAMES) == 553


def test_block_sizes_sum_to_total():
    assert sum(s.stop - s.start for s in OC.BLOCK_SLICES.values()) == 553


def test_zeros_observation_valid():
    z = B.zeros()
    assert z.shape == (553,) and z.dtype == np.float32 and np.all(np.isfinite(z))
...
def test_full_build_finite_and_shaped():
    ...
    assert obs.shape == (553,) and obs.dtype == np.float32 and np.all(np.isfinite(obs))
