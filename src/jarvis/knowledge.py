...
PIPELINE (data -> decision):
  ...
  src/env/           TradingEnv: the locked 517-float observation (contract v1.8.0) + actions
...
THREE RULES THAT OVERRIDE EVERYTHING (CLAUDE.md): (1) never silently change the 517 observation shape;
...
TROUBLESHOOTING = [
    ...
    {"id": "train-obs-shape", "area": "obs",
     "symptom": "shape mismatch error / 'expected 517 got N' / a saved model won't load",
     "cause": "the observation contract changed (someone edited a block size or MAX_STRATEGIES)",
     "fix": "the obs is LOCKED at 517 float32 (contract v1.8.0). Rule #1: never change it silently. If a "
    ...
    {"id": "portfolio-trader", "area": "trading",
     ...
     "fix": "it is a PORTFOLIO trader — ONE shared equity/drawdown pot across the WHOLE FTMO universe. Train it "
            ...
            "context, the obs stays 517 and it scales to the full FTMO broker list live. See "
